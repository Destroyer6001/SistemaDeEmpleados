...
<div class="block mx-auto my-40 p-8 bg-white w-1/3 border border-gray-200 rounded-lg shadow-lg">
        ...
        @endif
        <form action="{{route('empleados.update',['id' => $empleado->id])}}" class="mt-8" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')

            <input type="text" name="Nombre" placeholder="Nombre"
            id="Nombre" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('Nombre',$empleado->Nombre)}}">
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('PrimerApellido',$empleado->PrimerApellido)}}">
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('SegundoApellido',$empleado->SegundoApellido)}}">

            <input type="email" name="Email" placeholder="Email"
            id="Email" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('Email',$empleado->Email)}}">
            ...
            <option value="">Seleccione una puesto</option>
                @foreach($puestos as $puesto)
                    <option value="{{$puesto->id}}" {{ (old('puestoid', $empleado->PuestoId) == $puesto->id) ? 'selected' : '' }}>
                        {{$puesto->Nombre}}
                    </option>
                @endforeach
            </select>
            ...
            @if(isset($empleado->Fotografia))
                <img class="img-thiumbnail img-fluid" src="{{asset('storage').'/'.$empleado->Fotografia}}"  width="50" alt="fotografia del empleado">
            @endif
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="">
